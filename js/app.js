...
***************************************************************************** */
    ...
    function set_timer(block, time_end, function_result) {
        let timer, start, end, _second = 1e3, _minute = _second * 60, _hour = _minute * 60, _day = _hour * 24, set_storage = () => {
            if (!localStorage.getItem("timer_start_" + set_timer.count)) {
                let now = new Date;
                localStorage.setItem("timer_start_" + set_timer.count, Math.round(now.getTime() / 1e3));
            }
        }, update_settings = () => {
            start = end = localStorage.getItem("timer_start_" + set_timer.count);
            end = new Date(+end * 1e3);
            ...
            let days = Math.floor(distance / _day), hours = Math.floor(distance % _day / _hour), minutes = Math.floor(distance % _hour / _minute), seconds = Math.floor(distance % _minute / _second);
            return [ days, hours, minutes, seconds ];
        }, set_values = () => {
            let now = new Date, distance = end - +now;
            ...
            if (distance <= 0) {
                ...
                minutesCircle.style.animationDuration = `${timer[2]}` * 60 + "s";
                minutesCircle.style.animationDelay = 0 + "s";
                hoursCircle.style.animationDuration = `${timer[1]}` * (60 * 60) + "s";
                hoursCircle.style.animationDelay = 0 + "s";
                daysCircle.style.animationDuration = `${timer[0]}` * (60 * 60 * 24) + "s";
                daysCircle.style.animationDelay = 0 + "s";
            }
        }, init = () => {
            set_timer.count == void 0 ? set_timer.count = 1 : set_timer.count++;
            set_storage();
            update_settings();
            timer = setInterval(set_values, 1e3);
            return timer;
        };
        return init();
    }
    ...
    function windowLoad() {
        ...
        document.querySelector(".watch__play-again-button");
        const wistiaVideo = document.querySelector(".watch__wistia");
        const videoCover = document.querySelector(".watch__cover");
        const video = document.getElementById("wistia_simple_video_90");
        if (video !== null) console.log("yes"); else console.log("no");
        ...
        document.getElementById("big-timer");
    }
    window["FLS"] = true;
    isWebp();
    addLoadedClass();
